...
const getproduct = async (req, res) => {
  try {
    const qry = `SELECT tbl_product.product_id,tbl_product.product_name,
        tbl_product.MRP,tbl_product.description,
...
const delete_vehical_cat = async (req, res) => {
  try {
    ...
    const qry = "DELETE FROM tbl_product WHERE product_id =?";
    await conn.query(qry, data, (err, result) => {
      if (err) {
        console.log(err.sqlMessage);
      } else {
...
const update_vehical_cat = async (req, res) => {
  try {
    const data = [req.params.product_id];
    const qry = "UPDATE tbl_product SET ? where product_id =?";
    await conn.query(qry, data, (err, result) => {
      if (err) {
        console.log(err.sqlMessage);
      } else {
...
module.exports = { getproduct, postproduct, delete_vehical_cat };
