...
function View_product_Category() {
  ...
  const getdata = async () => {
    ...
    // );
    const Response = await fetch("http://localhost:4000/viewproduct_category", {
      method: "GET",
    });
  ...
  const deleteuser = async (id) => {
    if (window.confirm(`You want to delete data of Id no: =  ${id} `))
      await axios.delete(`http://localhost:4000/deleteproduct_category/${id}`);
    getdata();
  };
  const columns = [
    {
      ...
      },
    {
      ...
      render: (data) => {
        return (
          <>
            <Link to={`/updateproductcategory/${data.product_cat_id}`}>
              <EditFilled
            ...
            <DeleteFilled
              ...
              onClick={() => {
                deleteuser(data.product_cat_id);
              }}
            />
          </>
        );
...
export default View_product_Category;
